...
# - Verifies AppKit / Foundation / CoreGraphics frameworks
...
ensure_appkit_macos() {
    ...
    local frameworks=(Foundation AppKit CoreGraphics)
    local fw
...
if ! ./configure --enable-appkit >>"$LOG_FILE" 2>&1; then
    show_failure_logs
...
# Copied Mach-O pages invalidate the linker-signed adhoc seal → SIGKILL (Code Signature Invalid).
$SUDO xattr -cr "${PHP_EXT_DIR}/${EXTENSION_NAME}.so" 2>/dev/null || true
$SUDO codesign --force --sign - "${PHP_EXT_DIR}/${EXTENSION_NAME}.so"
ok "Copied and adhoc-signed: ${PHP_EXT_DIR}/${EXTENSION_NAME}.so"
echo ""
...
echo ""
echo "Demo:"
echo "  php examples/proof_appkit.php"
echo ""
echo "File locations:"
