
#ifdef HAVE_CONFIG_H
#include "../../../ext_config.h"
#endif

#include <php.h>
#include "../../../php_ext.h"
#include "../../../ext.h"

#include <Zend/zend_operators.h>
#include <Zend/zend_exceptions.h>
#include <Zend/zend_interfaces.h>

#include "kernel/main.h"
#include "kernel/operators.h"
#include "kernel/memory.h"
#include "kernel/object.h"

#include "ns-tabview.h"
#include <stdint.h>



/**
 * NSTabView tabbed container.
 */
ZEPHIR_INIT_CLASS(AppKit_NS_NSTabView_NSTabView)
{
	ZEPHIR_REGISTER_CLASS(AppKit\\NS\\NSTabView, NSTabView, appkit, ns_nstabview_nstabview, appkit_ns_nstabview_nstabview_method_entry, 0);

	return SUCCESS;
}

PHP_METHOD(AppKit_NS_NSTabView_NSTabView, create)
{
	zval *x_param = NULL, *y_param = NULL, *width_param = NULL, *height_param = NULL;
	zend_long x, y, width, height, handle = 0;

	ZEND_PARSE_PARAMETERS_START(4, 4)
		Z_PARAM_LONG(x)
		Z_PARAM_LONG(y)
		Z_PARAM_LONG(width)
		Z_PARAM_LONG(height)
	ZEND_PARSE_PARAMETERS_END();
	zephir_fetch_params_without_memory_grow(4, 0, &x_param, &y_param, &width_param, &height_param);
	
            handle = (zend_long) ns_tabview_create((double) x, (double) y, (double) width, (double) height);
        
	RETURN_LONG(handle);
}

PHP_METHOD(AppKit_NS_NSTabView_NSTabView, destroy)
{
	zval *tabs_param = NULL;
	zend_long tabs;

	ZEND_PARSE_PARAMETERS_START(1, 1)
		Z_PARAM_LONG(tabs)
	ZEND_PARSE_PARAMETERS_END();
	zephir_fetch_params_without_memory_grow(1, 0, &tabs_param);
	
            ns_tabview_destroy((uintptr_t) tabs);
        
}

PHP_METHOD(AppKit_NS_NSTabView_NSTabView, addTab)
{
	zephir_method_globals *ZEPHIR_METHOD_GLOBALS_PTR = NULL;
	zval title;
	zval *tabs_param = NULL, *title_param = NULL, *child_param = NULL;
	zend_long tabs, child;

	ZVAL_UNDEF(&title);
	ZEND_PARSE_PARAMETERS_START(3, 3)
		Z_PARAM_LONG(tabs)
		Z_PARAM_STR(title)
		Z_PARAM_LONG(child)
	ZEND_PARSE_PARAMETERS_END();
	ZEPHIR_METHOD_GLOBALS_PTR = pecalloc(1, sizeof(zephir_method_globals), 0);
	zephir_memory_grow_stack(ZEPHIR_METHOD_GLOBALS_PTR, __func__);
	zephir_fetch_params(1, 3, 0, &tabs_param, &title_param, &child_param);
	zephir_get_strval(&title, title_param);
	
            ns_tabview_add_tab((uintptr_t) tabs, Z_STRVAL(title), (uintptr_t) child);
        
	ZEPHIR_MM_RESTORE();
}

PHP_METHOD(AppKit_NS_NSTabView_NSTabView, getSelected)
{
	zval *tabs_param = NULL;
	zend_long tabs, value = 0;

	ZEND_PARSE_PARAMETERS_START(1, 1)
		Z_PARAM_LONG(tabs)
	ZEND_PARSE_PARAMETERS_END();
	zephir_fetch_params_without_memory_grow(1, 0, &tabs_param);
	
            value = (zend_long) ns_tabview_get_selected((uintptr_t) tabs);
        
	RETURN_LONG(value);
}

PHP_METHOD(AppKit_NS_NSTabView_NSTabView, setSelected)
{
	zval *tabs_param = NULL, *index_param = NULL;
	zend_long tabs, index;

	ZEND_PARSE_PARAMETERS_START(2, 2)
		Z_PARAM_LONG(tabs)
		Z_PARAM_LONG(index)
	ZEND_PARSE_PARAMETERS_END();
	zephir_fetch_params_without_memory_grow(2, 0, &tabs_param, &index_param);
	
            ns_tabview_set_selected((uintptr_t) tabs, (int) index);
        
}

