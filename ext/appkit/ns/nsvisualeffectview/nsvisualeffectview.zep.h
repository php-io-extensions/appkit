
extern zend_class_entry *appkit_ns_nsvisualeffectview_nsvisualeffectview_ce;

ZEPHIR_INIT_CLASS(AppKit_NS_NSVisualEffectView_NSVisualEffectView);

PHP_METHOD(AppKit_NS_NSVisualEffectView_NSVisualEffectView, initWithFrame);
PHP_METHOD(AppKit_NS_NSVisualEffectView_NSVisualEffectView, material);
PHP_METHOD(AppKit_NS_NSVisualEffectView_NSVisualEffectView, setMaterial);
PHP_METHOD(AppKit_NS_NSVisualEffectView_NSVisualEffectView, interiorBackgroundStyle);
PHP_METHOD(AppKit_NS_NSVisualEffectView_NSVisualEffectView, blendingMode);
PHP_METHOD(AppKit_NS_NSVisualEffectView_NSVisualEffectView, setBlendingMode);
PHP_METHOD(AppKit_NS_NSVisualEffectView_NSVisualEffectView, state);
PHP_METHOD(AppKit_NS_NSVisualEffectView_NSVisualEffectView, setState);
PHP_METHOD(AppKit_NS_NSVisualEffectView_NSVisualEffectView, maskImage);
PHP_METHOD(AppKit_NS_NSVisualEffectView_NSVisualEffectView, setMaskImage);
PHP_METHOD(AppKit_NS_NSVisualEffectView_NSVisualEffectView, isEmphasized);
PHP_METHOD(AppKit_NS_NSVisualEffectView_NSVisualEffectView, setEmphasized);
PHP_METHOD(AppKit_NS_NSVisualEffectView_NSVisualEffectView, viewDidMoveToWindow);
PHP_METHOD(AppKit_NS_NSVisualEffectView_NSVisualEffectView, viewWillMoveToWindow);

ZEND_BEGIN_ARG_WITH_RETURN_TYPE_INFO_EX(arginfo_appkit_ns_nsvisualeffectview_nsvisualeffectview_initwithframe, 0, 4, IS_LONG, 0)
	ZEND_ARG_TYPE_INFO(0, x, IS_DOUBLE, 0)
	ZEND_ARG_TYPE_INFO(0, y, IS_DOUBLE, 0)
	ZEND_ARG_TYPE_INFO(0, width, IS_DOUBLE, 0)
	ZEND_ARG_TYPE_INFO(0, height, IS_DOUBLE, 0)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_WITH_RETURN_TYPE_INFO_EX(arginfo_appkit_ns_nsvisualeffectview_nsvisualeffectview_material, 0, 1, IS_LONG, 0)
	ZEND_ARG_TYPE_INFO(0, handle, IS_LONG, 0)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_WITH_RETURN_TYPE_INFO_EX(arginfo_appkit_ns_nsvisualeffectview_nsvisualeffectview_setmaterial, 0, 2, IS_VOID, 0)

	ZEND_ARG_TYPE_INFO(0, handle, IS_LONG, 0)
	ZEND_ARG_TYPE_INFO(0, material, IS_LONG, 0)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_WITH_RETURN_TYPE_INFO_EX(arginfo_appkit_ns_nsvisualeffectview_nsvisualeffectview_interiorbackgroundstyle, 0, 1, IS_LONG, 0)
	ZEND_ARG_TYPE_INFO(0, handle, IS_LONG, 0)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_WITH_RETURN_TYPE_INFO_EX(arginfo_appkit_ns_nsvisualeffectview_nsvisualeffectview_blendingmode, 0, 1, IS_LONG, 0)
	ZEND_ARG_TYPE_INFO(0, handle, IS_LONG, 0)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_WITH_RETURN_TYPE_INFO_EX(arginfo_appkit_ns_nsvisualeffectview_nsvisualeffectview_setblendingmode, 0, 2, IS_VOID, 0)

	ZEND_ARG_TYPE_INFO(0, handle, IS_LONG, 0)
	ZEND_ARG_TYPE_INFO(0, blendingMode, IS_LONG, 0)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_WITH_RETURN_TYPE_INFO_EX(arginfo_appkit_ns_nsvisualeffectview_nsvisualeffectview_state, 0, 1, IS_LONG, 0)
	ZEND_ARG_TYPE_INFO(0, handle, IS_LONG, 0)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_WITH_RETURN_TYPE_INFO_EX(arginfo_appkit_ns_nsvisualeffectview_nsvisualeffectview_setstate, 0, 2, IS_VOID, 0)

	ZEND_ARG_TYPE_INFO(0, handle, IS_LONG, 0)
	ZEND_ARG_TYPE_INFO(0, state, IS_LONG, 0)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_WITH_RETURN_TYPE_INFO_EX(arginfo_appkit_ns_nsvisualeffectview_nsvisualeffectview_maskimage, 0, 1, IS_LONG, 0)
	ZEND_ARG_TYPE_INFO(0, handle, IS_LONG, 0)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_WITH_RETURN_TYPE_INFO_EX(arginfo_appkit_ns_nsvisualeffectview_nsvisualeffectview_setmaskimage, 0, 2, IS_VOID, 0)

	ZEND_ARG_TYPE_INFO(0, handle, IS_LONG, 0)
	ZEND_ARG_TYPE_INFO(0, maskImage, IS_LONG, 0)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_WITH_RETURN_TYPE_INFO_EX(arginfo_appkit_ns_nsvisualeffectview_nsvisualeffectview_isemphasized, 0, 1, _IS_BOOL, 0)
	ZEND_ARG_TYPE_INFO(0, handle, IS_LONG, 0)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_WITH_RETURN_TYPE_INFO_EX(arginfo_appkit_ns_nsvisualeffectview_nsvisualeffectview_setemphasized, 0, 2, IS_VOID, 0)

	ZEND_ARG_TYPE_INFO(0, handle, IS_LONG, 0)
	ZEND_ARG_TYPE_INFO(0, emphasized, _IS_BOOL, 0)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_WITH_RETURN_TYPE_INFO_EX(arginfo_appkit_ns_nsvisualeffectview_nsvisualeffectview_viewdidmovetowindow, 0, 1, IS_VOID, 0)

	ZEND_ARG_TYPE_INFO(0, handle, IS_LONG, 0)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_WITH_RETURN_TYPE_INFO_EX(arginfo_appkit_ns_nsvisualeffectview_nsvisualeffectview_viewwillmovetowindow, 0, 2, IS_VOID, 0)

	ZEND_ARG_TYPE_INFO(0, handle, IS_LONG, 0)
	ZEND_ARG_TYPE_INFO(0, newWindow, IS_LONG, 0)
ZEND_END_ARG_INFO()

ZEPHIR_INIT_FUNCS(appkit_ns_nsvisualeffectview_nsvisualeffectview_method_entry) {
	PHP_ME(AppKit_NS_NSVisualEffectView_NSVisualEffectView, initWithFrame, arginfo_appkit_ns_nsvisualeffectview_nsvisualeffectview_initwithframe, ZEND_ACC_PUBLIC|ZEND_ACC_STATIC)
	PHP_ME(AppKit_NS_NSVisualEffectView_NSVisualEffectView, material, arginfo_appkit_ns_nsvisualeffectview_nsvisualeffectview_material, ZEND_ACC_PUBLIC|ZEND_ACC_STATIC)
	PHP_ME(AppKit_NS_NSVisualEffectView_NSVisualEffectView, setMaterial, arginfo_appkit_ns_nsvisualeffectview_nsvisualeffectview_setmaterial, ZEND_ACC_PUBLIC|ZEND_ACC_STATIC)
	PHP_ME(AppKit_NS_NSVisualEffectView_NSVisualEffectView, interiorBackgroundStyle, arginfo_appkit_ns_nsvisualeffectview_nsvisualeffectview_interiorbackgroundstyle, ZEND_ACC_PUBLIC|ZEND_ACC_STATIC)
	PHP_ME(AppKit_NS_NSVisualEffectView_NSVisualEffectView, blendingMode, arginfo_appkit_ns_nsvisualeffectview_nsvisualeffectview_blendingmode, ZEND_ACC_PUBLIC|ZEND_ACC_STATIC)
	PHP_ME(AppKit_NS_NSVisualEffectView_NSVisualEffectView, setBlendingMode, arginfo_appkit_ns_nsvisualeffectview_nsvisualeffectview_setblendingmode, ZEND_ACC_PUBLIC|ZEND_ACC_STATIC)
	PHP_ME(AppKit_NS_NSVisualEffectView_NSVisualEffectView, state, arginfo_appkit_ns_nsvisualeffectview_nsvisualeffectview_state, ZEND_ACC_PUBLIC|ZEND_ACC_STATIC)
	PHP_ME(AppKit_NS_NSVisualEffectView_NSVisualEffectView, setState, arginfo_appkit_ns_nsvisualeffectview_nsvisualeffectview_setstate, ZEND_ACC_PUBLIC|ZEND_ACC_STATIC)
	PHP_ME(AppKit_NS_NSVisualEffectView_NSVisualEffectView, maskImage, arginfo_appkit_ns_nsvisualeffectview_nsvisualeffectview_maskimage, ZEND_ACC_PUBLIC|ZEND_ACC_STATIC)
	PHP_ME(AppKit_NS_NSVisualEffectView_NSVisualEffectView, setMaskImage, arginfo_appkit_ns_nsvisualeffectview_nsvisualeffectview_setmaskimage, ZEND_ACC_PUBLIC|ZEND_ACC_STATIC)
	PHP_ME(AppKit_NS_NSVisualEffectView_NSVisualEffectView, isEmphasized, arginfo_appkit_ns_nsvisualeffectview_nsvisualeffectview_isemphasized, ZEND_ACC_PUBLIC|ZEND_ACC_STATIC)
	PHP_ME(AppKit_NS_NSVisualEffectView_NSVisualEffectView, setEmphasized, arginfo_appkit_ns_nsvisualeffectview_nsvisualeffectview_setemphasized, ZEND_ACC_PUBLIC|ZEND_ACC_STATIC)
	PHP_ME(AppKit_NS_NSVisualEffectView_NSVisualEffectView, viewDidMoveToWindow, arginfo_appkit_ns_nsvisualeffectview_nsvisualeffectview_viewdidmovetowindow, ZEND_ACC_PUBLIC|ZEND_ACC_STATIC)
	PHP_ME(AppKit_NS_NSVisualEffectView_NSVisualEffectView, viewWillMoveToWindow, arginfo_appkit_ns_nsvisualeffectview_nsvisualeffectview_viewwillmovetowindow, ZEND_ACC_PUBLIC|ZEND_ACC_STATIC)
	PHP_FE_END
};
