...
{
	ZEPHIR_REGISTER_CLASS(AppKit\\NS\\NSTokenField, NSTokenField, appkit, ns_nstokenfield_nstokenfield, appkit_ns_nstokenfield_nstokenfield_method_entry, 0);
...
{
	zval *field_param = NULL, *style_param = NULL;
	zend_long field, style;

	ZEND_PARSE_PARAMETERS_START(2, 2)
		Z_PARAM_LONG(field)
		Z_PARAM_LONG(style)
	ZEND_PARSE_PARAMETERS_END();
	zephir_fetch_params_without_memory_grow(2, 0, &field_param, &style_param);
...
{
	double delay;
	zval *field_param = NULL, *delay_param = NULL;
	zend_long field;
	...
	ZEND_PARSE_PARAMETERS_START(2, 2)
		...
		Z_PARAM_ZVAL(delay)
	ZEND_PARSE_PARAMETERS_END();
	zephir_fetch_params_without_memory_grow(2, 0, &field_param, &delay_param);
