...
<?php
/**
 * Verify src/ns-responder.h against inventory + live SDK count.
 * Modes:
 *   (default)        bound+reserved == live expected, matches inventory
 *   --prototypes     every @zep is followed by a matching ns_nsresponder_* prototype
 *   --no-protocol    NSStandardKeyBindingResponding selectors are absent
 */

declare(strict_types=1);

$root = dirname(__DIR__, 2);
$headerPath = $root . '/src/ns-responder.h';
$inventoryPath = $root . '/.unlazy/sz-nsresponder/inventory.json';
...
foreach ($lines as $i => $line) {
    if (preg_match('#/\*\s*@zep\s+NS\\\\NSResponder\s+(\w+)\s*\(([^)]*)\)\s*->\s*(\w+)\s*\*/#', $line, $m)) {
        $bound++;
        $params = trim($m[2]) === '' ? [] : array_map('trim', explode(',', $m[2]));
        $zep[] = ['line' => $i, 'method' => $m[1], 'params' => $params, 'return' => $m[3]];
    } elseif (preg_match('#/\*\s*@reserved\s+NS\\\\NSResponder\s#', $line)) {
        $reserved++;
    }
}
...
if ($mode === '--no-protocol') {
    $forbidden = [
        'insertText', 'doCommandBySelector', 'moveForward', 'moveRight',
        'moveBackward', 'moveLeft', 'selectAll', 'deleteForward', 'deleteBackward',
        'quickLookPreviewItems', 'showContextMenuForSelection',
    ];
    $hits = [];
    foreach ($zep as $z) {
    ...
    if ($hits !== []) {
        fwrite(STDERR, "protocol selectors bound: " . implode(', ', $hits) . "\n");
        exit(1);
    }
    echo "NSRESPONDER_NO_PROTOCOL_OK\n";
    exit(0);
}
...
if ($mode === '--prototypes') {
    foreach ($zep as $z) {
        ...
        if (is_null($proto) || !preg_match('/^\s*(zend_long|double|void)\s+(ns_nsresponder_[a-z0-9_]+)\s*\(([^)]*)\)\s*;/', $proto, $pm)) {
        ...
        if (!str_starts_with($pm[2], 'ns_nsresponder_')) {
            fwrite(STDERR, "{$z['method']}: symbol {$pm[2]} is not ns_nsresponder_*\n");
            exit(1);
        }
    }
    if ($zep === []) {
    ...
    echo "NSRESPONDER_PROTOTYPES_OK\n";
    exit(0);
}
