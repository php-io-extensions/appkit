...
<?php
/**
 * Classify NSControl.h members and compare against the live SDK count.
 * Modes:
...
$header = '/Library/Developer/CommandLineTools/SDKs/MacOSX.sdk/System/Library/Frameworks/AppKit.framework/Headers/NSControl.h';
$mode = $argv[1] ?? 'slots';
...
$cmd = escapeshellarg(PHP_BINARY) . ' ' . escapeshellarg($root . '/scripts/audit-headers.php')
    . ' --count ' . escapeshellarg($header) . ' NSControl';
exec($cmd, $out, $code);
$line = implode("\n", $out);
...
if ($mode === '--reserved-reasons') {
    ...
    echo "NSCONTROL_RESERVED_REASONS_OK\n";
    exit(0);
}
