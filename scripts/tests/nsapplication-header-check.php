...
<?php
/**
 * Verify src/ns-application.h against inventory + live SDK count.
 * Modes:
 *   (default)        bound+reserved == live expected, matches inventory
 *   --prototypes     every @zep is followed by a matching ns_nsapplication_* prototype
...
$headerPath = $root . '/src/ns-application.h';
$inventoryPath = $root . '/.unlazy/sz-nsapplication/inventory.json';
...
foreach ($lines as $i => $line) {
    if (preg_match('#/\*\s*@zep\s+NS\\\\NSApplication\s+(\w+)\s*\(([^)]*)\)\s*->\s*(\w+)\s*\*/#', $line, $m)) {
        $bound++;
        $params = trim($m[2]) === '' ? [] : array_map('trim', explode(',', $m[2]));
        $zep[] = ['line' => $i, 'method' => $m[1], 'params' => $params, 'return' => $m[3]];
    } elseif (preg_match('#/\*\s*@reserved\s+NS\\\\NSApplication\s#', $line)) {
        $reserved++;
    }
}
...
if ($mode === '--no-inherited') {
    ...
    echo "NSAPPLICATION_NO_INHERITED_OK\n";
    exit(0);
}
...
if ($mode === '--prototypes') {
    foreach ($zep as $z) {
        ...
        if ($proto === null || !preg_match('/^\s*(zend_long|double|void)\s+(ns_nsapplication_[a-z0-9_]+)\s*\(([^)]*)\)\s*;/', $proto, $pm)) {
        ...
        if (!str_starts_with($pm[2], 'ns_nsapplication_')) {
            fwrite(STDERR, "{$z['method']}: symbol {$pm[2]} is not ns_nsapplication_*\n");
            exit(1);
        }
    }
    if ($zep === []) {
    ...
    echo "NSAPPLICATION_PROTOTYPES_OK\n";
    exit(0);
}
