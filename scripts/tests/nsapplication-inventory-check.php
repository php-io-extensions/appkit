...
<?php
/**
 * Classify NSApplication.h members and compare against the live SDK count.
 * Modes:
 *   (default)               inventory slots == audit-headers expected
 *   --reserved-reasons      every reserved row is deprecated or block
 */
...
$header = '/Library/Developer/CommandLineTools/SDKs/MacOSX.sdk/System/Library/Frameworks/AppKit.framework/Headers/NSApplication.h';
$mode = $argv[1] ?? 'slots';
...
$cmd = escapeshellarg(PHP_BINARY) . ' ' . escapeshellarg($root . '/scripts/audit-headers.php')
    . ' --count ' . escapeshellarg($header) . ' NSApplication';
exec($cmd, $out, $code);
$line = implode("\n", $out);
...
foreach ($inventory['members'] as $i => $row) {
    ...
    if ($disp === 'reserved') {
        ...
        if ($mode === '--reserved-reasons' && !in_array($reason, ['deprecated', 'block'], true)) {
            fwrite(STDERR, "reserved member[{$i}] reason '{$reason}' is not deprecated|block\n");
            exit(1);
        }
    }
}
...
if ($mode === '--reserved-reasons') {
    ...
    echo "NSAPPLICATION_RESERVED_REASONS_OK\n";
    exit(0);
}
...
echo "NSAPPLICATION_INVENTORY_OK\n";
