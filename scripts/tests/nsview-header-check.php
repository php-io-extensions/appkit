...
<?php
/**
 * Verify src/ns-view.h against inventory + live SDK count.
 * Modes:
 *   (default)        bound+reserved == live expected, matches inventory
 *   --prototypes     every @zep is followed by a matching ns_nsview_* prototype
...
foreach ($lines as $i => $line) {
    if (preg_match('#/\*\s*@zep\s+NS\\\\NSView\s+(\w+)\s*\(([^)]*)\)\s*->\s*(\w+)\s*\*/#', $line, $m)) {
        $bound++;
        $params = trim($m[2]) === '' ? [] : array_map('trim', explode(',', $m[2]));
        $zep[] = ['line' => $i, 'method' => $m[1], 'params' => $params, 'return' => $m[3]];
    } elseif (preg_match('#/\*\s*@reserved\s+NS\\\\NSView\s#', $line)) {
        $reserved++;
    }
}
...
if ($mode === '--prototypes') {
    foreach ($zep as $z) {
        ...
        if ($proto === null || !preg_match('/^\s*(zend_long|double|void)\s+(ns_nsview_[a-z0-9_]+)\s*\(([^)]*)\)\s*;/', $proto, $pm)) {
        ...
        if (!str_starts_with($pm[2], 'ns_nsview_')) {
            fwrite(STDERR, "{$z['method']}: symbol {$pm[2]} is not ns_nsview_*\n");
            exit(1);
        }
    }
    if ($zep === []) {
