...
<?php
/**
 * Classify NSResponder.h members and compare against the live SDK count.
 * Modes:
...
$header = '/Library/Developer/CommandLineTools/SDKs/MacOSX.sdk/System/Library/Frameworks/AppKit.framework/Headers/NSResponder.h';
$mode = $argv[1] ?? 'slots';
...
$cmd = escapeshellarg(PHP_BINARY) . ' ' . escapeshellarg($root . '/scripts/audit-headers.php')
    . ' --count ' . escapeshellarg($header) . ' NSResponder';
exec($cmd, $out, $code);
$line = implode("\n", $out);
...
if ($mode === '--reserved-reasons') {
    ...
    echo "NSRESPONDER_RESERVED_REASONS_OK\n";
    exit(0);
}
