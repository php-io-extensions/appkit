...
<?php
/**
 * Verify src/ns-control.h against inventory + live SDK count.
 * Modes:
 *   (default)        bound+reserved == live expected, matches inventory
 *   --prototypes     every @zep is followed by a matching ns_nscontrol_* prototype
 *   --no-inherited   NSView/NSResponder.h-only selectors are absent
 */

declare(strict_types=1);

$root = dirname(__DIR__, 2);
$headerPath = $root . '/src/ns-control.h';
$inventoryPath = $root . '/.unlazy/sz-nscontrol/inventory.json';
...
foreach ($lines as $i => $line) {
    if (preg_match('#/\*\s*@zep\s+NS\\\\NSControl\s+(\w+)\s*\(([^)]*)\)\s*->\s*(\w+)\s*\*/#', $line, $m)) {
        $bound++;
        $params = trim($m[2]) === '' ? [] : array_map('trim', explode(',', $m[2]));
        $zep[] = ['line' => $i, 'method' => $m[1], 'params' => $params, 'return' => $m[3]];
    } elseif (preg_match('#/\*\s*@reserved\s+NS\\\\NSControl\s#', $line)) {
        $reserved++;
    }
}
...
if ($mode === '--no-inherited') {
    ...
    if ($hits !== []) {
        fwrite(STDERR, "inherited NSView/NSResponder selectors bound: " . implode(', ', $hits) . "\n");
        exit(1);
    }
    echo "NSCONTROL_NO_INHERITED_OK\n";
    exit(0);
}
...
if ($mode === '--prototypes') {
    foreach ($zep as $z) {
        ...
        if ($proto === null || !preg_match('/^\s*(zend_long|double|void)\s+(ns_nscontrol_[a-z0-9_]+)\s*\(([^)]*)\)\s*;/', $proto, $pm)) {
        ...
        if (!str_starts_with($pm[2], 'ns_nscontrol_')) {
            fwrite(STDERR, "{$z['method']}: symbol {$pm[2]} is not ns_nscontrol_*\n");
            exit(1);
        }
    }
    if ($zep === []) {
