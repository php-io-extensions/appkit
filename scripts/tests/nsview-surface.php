...
<?php
/**
 * End-to-end NSView C-surface checks.
 * Modes:
 *   (default)         bound+reserved == live SDK count; every @zep has a .m body
 *   --composite       each ns_nsview_* body has at most one message send
 *                     (alloc+init allowed inside init* bindings)
 *   --extra-sources   config.json lists src/ns-view.m
 *   --no-nsphp        no NSPhp interface/class in ns-view.{h,m}
 */
...
$mode = $argv[1] ?? 'surface';
$hPath = $root . '/src/ns-view.h';
$mPath = $root . '/src/ns-view.m';
$sdk = '/Library/Developer/CommandLineTools/SDKs/MacOSX.sdk/System/Library/Frameworks/AppKit.framework/Headers/NSView.h';
...
if ($mode === '--extra-sources') {
    ...
    if (!in_array('src/ns-view.m', $listed, true)) {
        fail('config.json extra-sources does not list src/ns-view.m');
    }
    echo "NSVIEW_EXTRA_SOURCES_OK\n";
    exit(0);
}
...
$tmp = sys_get_temp_dir() . '/nsview-parity-lib.php';
...
if ($mode === '--composite') {
    ...
    if ($bodies === []) {
        fail('no function bodies in ns-view.m');
    }
    $hLines = file($hPath, FILE_IGNORE_NEW_LINES) ?: [];
    $symbolIsInit = [];
    foreach ($hLines as $i => $line) {
        if (!preg_match('#/\*\s*@zep\s+NS\\\\NSView\s+(\w+)\s*\(#', $line, $mm)) {
            continue;
        }
        $method = $mm[1];
        $proto = null;
        ...
        if ($proto !== null && preg_match('/\b(ns_nsview_[a-z0-9_]+)\s*\(/', $proto, $pm)) {
            $symbolIsInit[$pm[1]] = isInitMethod($method);
        }
    }
    foreach ($bodies as $symbol => $body) {
        if (!str_starts_with($symbol, 'ns_nsview_')) {
            continue;
        }
    ...
    echo "NSVIEW_COMPOSITE_OK\n";
    exit(0);
}

if (!is_file($hPath) || !is_file($mPath)) {
    fail('ns-view.{h,m} missing');
}

$h = (string) file_get_contents($hPath);
$bound = preg_match_all('#/\*\s*@zep\s+NS\\\\NSView\s+#', $h);
$reserved = preg_match_all('#/\*\s*@reserved\s+NS\\\\NSView\s+#', $h);

$cmd = escapeshellarg(PHP_BINARY) . ' ' . escapeshellarg($root . '/scripts/audit-headers.php')
    . ' --count ' . escapeshellarg($sdk) . ' NSView';
exec($cmd, $out, $code);
$line = implode("\n", $out);
...
foreach ($headerLines as $i => $line) {
    if (!preg_match('#/\*\s*@zep\s+NS\\\\NSView\s+#', $line)) {
        continue;
    }
    $proto = null;
    ...
    if ($proto === null || !preg_match('/\b(ns_nsview_[a-z0-9_]+)\s*\(/', $proto, $pm)) {
        fail('zep without ns_nsview_* prototype near line ' . ($i + 1));
    }
    $symbols[] = $pm[1];
}
...
$config = json_decode((string) file_get_contents($root . '/config.json'), true);
if (!in_array('src/ns-view.m', $config['extra-sources'] ?? [], true)) {
    fail('config.json extra-sources does not list src/ns-view.m');
}

echo "NSVIEW_SURFACE_OK\n";
