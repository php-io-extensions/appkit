...
#   6. strip phpize leftovers so PIE never ships host-absolute paths
#
# APPKIT_ROOT  — package root (default: parent of this script)
# ZEPHIR_BIN   — zephir executable (otherwise searched on PATH / composer)

set -Eeuo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT_DIR="${APPKIT_ROOT:-$(cd "${SCRIPT_DIR}/.." && pwd)}"
...
# Herd's php.ini loads appkit/metal; that segfaults `zephir generate`.
...
run_zephir_generate() {
    ...
    # ini cannot load a previous appkit.so and segfault generate.
...
write_makefile_frag() {
...

    {
        ...
        echo "NS_OBJC_CFLAGS = -fobjc-arc -x objective-c -Wno-deprecated-declarations"
        echo
        shopt -s nullglob
        for mfile in "${SRC_DIR}"/*.m; do
            ...
            printf '\t%s\n' "\$(LIBTOOL) --tag=CC --mode=compile \$(CC) -I. -I\$(srcdir) -I\$(srcdir)/src \$(COMMON_FLAGS) \$(CFLAGS_CLEAN) \$(EXTRA_CFLAGS) \$(NS_OBJC_CFLAGS) -DZEND_COMPILE_DL_EXT=1 -c \$(srcdir)/src/${base}.m -o \$@"
            echo
        done
        shopt -u nullglob
    } > "$frag"
...
#
# Nested namespaces (AppKit\Bridge, AppKit\NS\NSButton) make Zephir emit
#
#     ZEPHIR_REGISTER_CLASS(AppKit\\Bridge, Bridge, appkit, bridge, appkit_bridge_bridge_method_entry, 0);
#
# The macro builds the class-entry as lower_ns##_##name##_ce → appkit_bridge_ce,
# but the header declares appkit_bridge_bridge_ce. The method-entry symbol on
