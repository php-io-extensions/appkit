...
4. Every header member is either bound (`@zep`) or kept as a commented `@reserved` signature (blocks, `NSError**`, `NSCoder`, `Class`, `NSAttributedString`, `NSData`, deprecated). Nothing is silently omitted. Exception: a class the SDK deprecates *in its entirety* that is still the only OS path for a feature (`NSOpenGLPixelFormat`, `NSOpenGLContext`, `NSOpenGLView`) is bound in full under a visible `/*@audit deprecated-class <Class> <reason> */` marker, which `audit-headers.php` enforces both ways; deprecated members inside otherwise-live classes stay reserved.
